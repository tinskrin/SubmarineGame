...
class GameViewController: UIViewController {
	...
	private let currentGameControl = GameSettings.shared.currentControl
	private let podlodkaOffset: CGFloat = 20
	...
	@objc private func upButtonAction(sender: UITapGestureRecognizer){
		changePodlodkaPosition(on: -podlodkaOffset)
	}
	...
	private func setupConstraints(){
		...
		NSLayoutConstraint.activate(
			[
				upButton.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: space),
				upButton.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -space),
				...
				downButton.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -space),
	...
	private func gameStart() {
		setupControl(gameControl: currentGameControl)
		isGameStart = true
		blurView.isHidden = true
		waterAction()
	...
	private func setupControl(gameControl: GameType) {
		switch gameControl {
		case .acelerometer:
		...
		motionManager.startAccelerometerUpdates(to: .main) { [weak self] (data: CMAccelerometerData?, error: Error?) in
			...
			if let acceleration = data?.acceleration {
				...
				if acceleration.x >= self.devicePositionX - 0.1 {
					self.changePodlodkaPosition(on: -self.podlodkaOffset)
